...
pub fn syncGetVal(track: *const SyncTrack, row: f64) f64 {
...
pub fn syncFindKey(t: *const SyncTrack, row: u32) struct { index: u32, found: bool } {
    var lo = @as(usize, 0);
...
//#ifndef SYNC_PLAYER
pub fn syncSetKey(track: *SyncTrack, key: TrackKey) !void {
    const result = syncFindKey(track, key.row);
    if (!result.found) {
...
pub fn syncDelKey(track: *SyncTrack, pos: u32) !void {
    var result = syncFindKey(track, pos);
    assert(result.found);
